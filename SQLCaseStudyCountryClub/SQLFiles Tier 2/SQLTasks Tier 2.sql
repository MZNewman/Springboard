...
SELECT facility_name,
	CASE WHEN memid = 0 THEN 'Guest'
		ELSE CONCAT(firstname, ' ', surname) END AS member_name,
	CASE WHEN memid = 0 THEN guestcost * slots
		ELSE membercost * slots END AS cost
FROM (
	SELECT b.memid, b.starttime, b.slots, f.name AS facility_name, f.membercost, f.guestcost, m.firstname, m.surname
	FROM Bookings AS b
    	INNER JOIN Facilities AS f
			ON b.facid = f.facid
		INNER JOIN Members AS m
...
WHERE starttime LIKE '2012-09-14%'
	AND CASE WHEN memid = 0 THEN guestcost * slots > 30
			ELSE membercost * slots > 30 END
ORDER BY cost DESC;
